...
## 0.6.3
* 深入优化二进制时钟组件：
  + 修复圆点后方出现方形框的问题（覆盖 vuepress 默认主题给 table/td 加的边框）
  + 圆点亮起光晕（box-shadow）颜色改为动态取自 `dotColor`，熄灭圆点光晕取自 `dotBgColor`，自定义配色时光晕同步变色
  + 优化熄灭圆点（白点）视觉：加细边框与适度阴影，在浅色背景上也有清晰轮廓，形成整体点阵感
  + `binary-clock.md` 缩放示例拆分为「缩小」「放大」两个独立代码框，避免放大示例溢出
* 修复 `package.json` 构建脚本在 Node 22 下的 OpenSSL 兼容问题
...
* 优化二进制时钟组件：圆点改用伪元素方案渲染，任意缩放/视口下保持完美正圆；新增 `dotColor` / `dotBgColor` / `dotSize` / `textColor` / `showLabel` 自定义参数；圆点尺寸随视口自适应
