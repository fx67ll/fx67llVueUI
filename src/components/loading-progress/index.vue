<template>
	<div class='fx67ll-progress-box'
		:class="{ 'progress-transition': styleTransition, 'progress-display': styleDisplay }"
		:style="{ '--zIndex': zIndex, '--gradientAnimationTime': animationTime + 's', '--colorfulAnimationTime': animationTime * 4 + 's','--bgColor': bgColor }">
		
		<!-- tip -->
		<div class="fx67ll-progress-tip" :style="{'--textColor': textColor}">
			{{ isFinished ? '资源加载完成 ' : '资源加载中...... ' + progressNum + '%' }}</div>

		<!-- striped -->
		<div class='fx67ll-progress fx67ll-progress-striped'
		...
		<!-- gradient -->
		<div class='fx67ll-progress fx67ll-progress-gradient'
		...
		<!-- colorful -->
		<div class='fx67ll-progress fx67ll-progress-colorful' :style="{ '--progressNum': progressNum + '%' }"
			v-if="styleType === 'colorful'">
			<div class='fx67ll-progress-bar'></div>
		</div>
	</div>
</template>
		...
		props: {
			...
			progressNum: {
				...
				validator(num) {
					...
					return new RegExp('^(([1-9][0-9]|[1-9])(\.\d{1,2})?|0\.\d{1,2}|100)$').test(parseInt(num));
				}
			},
			// 加载是否已完成
			isFinished: {
			...
			animationTime: {
				...
				validator(time) {
					// 只要是正数即可
					return new RegExp('^[+]{0,1}[1-9]\d*$|^[+]{0,1}(0\.\d*[1-9])$|^[+]{0,1}([1-9]\d*\.\d*[1-9])$').test(
						parseFloat(time));
				}
			}
		},
...
<style lang="less">
	...
	.fx67ll-progress-box {
		...
		.fx67ll-progress-striped {
			...
			.fx67ll-progress-bar {
				...
				background-image: linear-gradient(45deg, @stripedSecondColor 25%, transparent 25%, transparent 50%, @stripedSecondColor 50%, @stripedSecondColor 75%, transparent 75%, transparent);
			}
		}
	...
	.keyframes(all, progressAnimationColorful, {
			0% {
				background-image: linear-gradient(to right, #4cd964, #5ac8fa, #007aff, #7DC8E8, #5856d6, #ff2d55);
			}
