<template>
	<div class="fx67ll-progress-box" :class="{ 'fx67ll-progress-transition': styleTransition, 'fx67ll-progress-display': styleDisplay }" :style="{ '--zIndex': zIndex, '--gradientAnimationTime': animationTime + 's', '--colorfulAnimationTime': animationTime * 4 + 's', '--bgColor': bgColor }">
		<!-- tip -->
		<div class="fx67ll-progress-tip" :style="{ '--textColor': textColor }" v-if="isShowText">{{ isFinished ? '资源加载完成 ' : '资源加载中...... ' + progressNum + '%' }}</div>

		<!-- striped -->
		<div class="fx67ll-progress fx67ll-progress-striped" :style="{ '--progressNum': progressNum + '%', '--stripedFirstColor': stripedFirstColor, '--stripedSecondColor': stripedSecondColor }" v-if="styleType === 'striped'">
			<div class="fx67ll-progress-bar"></div>
		</div>

		<!-- gradient -->
		<div class="fx67ll-progress fx67ll-progress-gradient" :style="{ '--progressNum': progressNum + '%', '--gradientFirstColor': gradientFirstColor, '--gradientSecondColor': gradientSecondColor }" v-if="styleType === 'gradient'">
			<div class="fx67ll-progress-bar"></div>
		</div>
			...
			progressNum: {
				...
				validator(num) {
					...
					if (parseInt(num) === 0) {
						...
						return new RegExp('^([0-9][0-9]{0,1}|100)$').test(parseInt(num))
					}
				}
			},
			// 加载是否已完成
...
<style lang="less">
	...
	.keyframes(all,
		...
		55% {
			background-image: linear-gradient(to left, #ff2d55, #4cd964, #5ac8fa, #007aff, #7dc8e8, #5856d6);
		}
